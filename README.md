...
│   ├── SearchService           — 파일 검색 (내용/파일명 대상, 와일드카드 패턴)
...
- 파일 실행
- 메인 메뉴에서 활성 패널 기준 Windows 탐색기 열기: 폴더 선택 시 해당 폴더를 열고, 파일 선택 시 상위 폴더에서 그 파일이 선택된 상태로 엶
...
- 파일 검색 (내용 검색/파일명 검색 대상 선택, 대/소문자 구분, 정규식, 하위 폴더 탐색 옵션)
  ...
  - 하위 폴더 포함 검색은 접근 권한이 없는 폴더를 건너뛰고 계속 진행
...
| 뷰어 | 기본 확장자 | 주요 기능 |
|---|---|---|
| Markdown | `.md`, `.markdown` | 미리보기·편집·분할 보기, Edit/Split 모드 라인번호 표시, 활성 탭 중심 파일 변경 자동 반영, TOC, front matter 표시, Mermaid, KaTeX, HTML/PDF 내보내기 |
...
- `보기 > 도킹 배치 초기화`를 실행하면 사라진 패널의 잠금 기록도 함께 정리됩니다.
