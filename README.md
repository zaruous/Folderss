...
│   ├── PinnedShortcutsPanel — 즐겨찾기 패널 위쪽 별도 레이어, 고정 바로가기 표시
...
- `보기 > 디스크 사용량 보기`로 드라이브별 사용량을 가로바와 GB 단위(총량/사용량/여유 공간)로 표시, 최초 실행 시 즐겨찾기 패널 위쪽 별도 레이어에 바로가기 고정
...
- 메뉴 클릭 시 즐겨찾기 패널(그룹 트리와 헤더 포함) 위쪽의 별도 레이어에 `디스크 사용량` 바로가기가 한 번만 고정 추가되며, 이후에는 이 바로가기를 클릭해도 같은 탭을 엽니다.
