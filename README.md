...
경로 탐색, 검색, 파일 목록은 재사용 가능한 `Controls/FolderBrowser` 컴포넌트 하나로 구성되며, 이 컴포넌트를 AvalonDock 레이아웃 위에 여러 개 배치합니다. 왼쪽 열에는 디스크 사용량 미니 패널과 즐겨찾기 패널이, 폴더 패널 아래에는 선택 파일 미리보기·메타정보 영역이 붙어 있습니다. 파일을 열 때는 확장자 매핑에 따라 Markdown, Monaco, Text 뷰어를 별도 탭으로 띄우거나 Windows 기본 프로그램으로 넘깁니다.
...
│   ├── DiskUsageMiniPanel  — 즐겨찾기 열 상단 도킹용 컴팩트 디스크 사용량 뷰
│   └── ViewerHost          — 내장 파일 뷰어 호스트 (다시 읽기 버튼, 뷰어 수명 관리)
├── Viewers/
│   ├── MarkdownViewer      — Markdown 미리보기·편집·검색·내보내기, 외부 변경 감시
│   ├── MonacoViewer        — Monaco Editor 기반 코드/텍스트 편집기, 외부 변경 감시
│   ├── TextViewer          — 읽기 전용 텍스트 뷰어
│   ├── IFileViewer         — 뷰어 공통 인터페이스
│   └── Resources/          — 뷰어 HTML/JS/CSS (marked, Mermaid, KaTeX, highlight.js, Monaco, CodeMirror)
├── Models/
...
│   └── SearchTarget        — 검색 대상 enum (내용/파일명)
├── Services/
...
- **내장 뷰어 시스템**: `IFileViewer` 인터페이스와 `ViewerHost`를 통해 Markdown, Monaco, Text 뷰어를 AvalonDock 문서 탭으로 엽니다. 같은 파일을 다시 열면 새 탭을 만들지 않고 기존 탭으로 이동합니다.
- **뷰어 외부 변경 감지**: Markdown·Monaco 뷰어는 `FileSystemWatcher`로 열린 파일의 외부 변경을 감지해 다시 읽습니다. 비활성 탭은 변경만 기록하고 탭이 활성화될 때 한 번만 반영합니다.
...
- **세션 복원**: 창을 트레이로 숨기기 직전과 실제 종료 직전에 열린 폴더 경로·활성 패널(`session.xml`)과 도킹 배치(`dock-layout.xml`)를 `%LOCALAPPDATA%\Folderss\`에 저장하고 다음 실행 시 복원합니다. 열려 있던 Markdown 뷰어 탭도 함께 복원됩니다.
- **사용자 설정**: 테마(`theme.txt`), 단축키(`keybindings.xml`), 뷰어 매핑(`viewer-config.json`), 열기 프로그램(`open-with.xml`), 콘솔 설정(`console-settings.xml`), 즐겨찾기(`favorites.xml`), 패널 잠금(`panel-locks.xml`)을 사용자별로 저장합니다.
- **테마**: `ResourceDictionary` 교체 방식으로 런타임에 즉시 전환합니다.
- **설정 저장 안정성**: 모든 설정 파일은 임시 파일에 쓴 뒤 교체하므로 저장 중 프로세스가 종료돼도 기존 파일이 손상되지 않습니다. 설정 창에서 저장이 실패하면 어떤 파일이 왜 실패했는지 메시지로 알리며, 변경 내용은 그 실행 중에는 그대로 적용됩니다.
...
- 좌우 듀얼 패널과 크기 조절
- 경로 직접 입력, 경로 입력란에 `Ctrl+V`로 경로 붙여넣기 (클립보드에 파일이 있으면 그 파일의 상위 폴더 경로가 들어감)
- 뒤로, 앞으로, 상위 폴더 이동 (`Backspace`도 상위 폴더로 이동)
- 현재 폴더 이름 검색
- 폴더 패널에서 일반 문자 입력 시 파일검색 필터로 자동 전달
- 파일검색 필터에서 `↓` 입력 시 파일 목록으로 포커스 이동
- 파일 실행 (`Enter` 또는 더블클릭)
...
- 다중 선택 파일 작업
- 클립보드 복사, 잘라내기, 붙여넣기
  - 복사·잘라내기 시 실제 파일(FileDrop)과 함께 이름 텍스트도 담기므로 텍스트 에디터에 붙여넣으면 파일 이름 목록이 들어감
  - 트리뷰에서 폴더를 선택한 상태로 `Ctrl+C`를 누르면 그 폴더를 복사 (폴더 이름 텍스트 + 실제 폴더)

### 트리뷰

- `보기 > 트리뷰`로 현재 폴더 패널 왼쪽에 하위 폴더 트리 표시
- 트리뷰 폴더 고정(📌): 트리뷰를 열면 기본으로 고정되며, 고정 중에는 트리 루트가 유지되고 해당 폴더 구조 변경(생성·삭제·이름변경·이동·붙여넣기)이 앱 안에서 차단됨 (토글로 해제 가능). 파일 이름 변경은 폴더 구조를 바꾸지 않으므로 고정 중에도 허용
- 트리뷰 폴더 우클릭 시 Windows 컨텍스트 메뉴 표시 (빈 영역은 트리 루트 기준)
- 트리뷰 새로고침(↻)은 선택한 폴더의 하위 트리만 다시 읽음 (선택이 없거나 선택 폴더가 삭제된 경우 전체 재구성)

### 패널·뷰어·도구

- AvalonDock 기반 패널 도킹, 탭, 분리 창, 자동 숨김
- 문서 탭 우클릭 메뉴: 닫기, 다른 탭 닫기, 왼쪽/오른쪽 탭 닫기, 패널 잠금, 탐색기로 열기
- `F11`로 현재 폴더 패널(또는 뷰어 탭) 최대화 및 복원
- 도킹 배치 자동 저장 및 복원
- 그룹별 즐겨찾기 구성과 사용자별 저장, 폴더뿐 아니라 파일도 즐겨찾기에 추가 가능
- 즐겨찾기 그룹 간 드래그 앤 드롭 이동
- 폴더 목록 하단의 선택 파일 미리보기와 메타정보
- 파일 검색 (내용 검색/파일명 검색 대상 선택, 대/소문자 구분, 정규식, 하위 폴더 탐색 옵션)
  - 검색 창을 열면 `파일명 검색`과 `하위 폴더 포함`이 기본으로 선택되어 있음
  - 파일명 검색에서 `*.cs`, `report?.txt` 같은 와일드카드 패턴 사용 (별도 확장자 필터 입력란을 대체)
  - `내용` 토글로 결과 목록의 일치한 줄 내용 컬럼을 켜고 끔
  - 하위 폴더 포함 검색은 접근 권한이 없는 폴더를 건너뛰고 계속 진행하며, 순환을 만드는 정션·심볼릭 링크에는 들어가지 않음
  - 대상 폴더 경로를 상시 표시하고, `폴더 선택…`으로 활성 패널과 무관한 폴더를 직접 지정 가능
    (직접 고르면 활성 패널을 따라가지 않으며, `현재 폴더` 버튼으로 해제)
- 내장 Markdown, Monaco, Text 뷰어 탭 (툴바의 `다시 읽기` 버튼, 외부 변경 자동 감지)
...
| 뷰어 | 기본 확장자 | 주요 기능 |
|---|---|---|
| Markdown | `.md`, `.markdown` | 미리보기·편집·분할 보기, Edit/Split 모드 라인번호 표시, `Ctrl+F` 문서 내 검색, TOC, front matter 표시, Mermaid, KaTeX, 상대 경로 이미지 표시, 로컬 파일 링크 클릭 시 새 뷰어 탭으로 열기, HTML/PDF 내보내기, 우클릭 메뉴의 인쇄·다른 이름으로 저장·공유, 외부 변경 자동 반영(스크롤 위치 유지) |
| Monaco | `.json`, `.xml` | Monaco Editor 기반 편집, 구문 강조, `Ctrl+F` 내장 검색(찾아 바꾸기 `Ctrl+H`, 커맨드 팔레트 `F1`), 대용량 파일 모드, 외부 변경 감지 |
| Text | 사용자 매핑 | 읽기 전용 텍스트 보기와 구문 강조 |

- 매핑되지 않은 확장자는 Windows 기본 프로그램으로 열립니다. `.cs`, `.py`, `.txt` 등은 설정에서 Monaco 또는 Text 뷰어에 추가로 매핑할 수 있습니다.
- 뷰어 탭 툴바의 `다시 읽기` 버튼으로 디스크 내용을 다시 불러올 수 있습니다.
- 뷰어 안에서 `Home`/`End` 단독 입력은 문서 탭 전환에 쓰지 않고 편집기에 그대로 전달됩니다.
- Markdown `HTML로 저장`으로 만든 파일은 앱 밖에서 열리므로 상대 경로 이미지는 표시되지 않습니다 (미리보기와 PDF 내보내기는 정상).

## 열기 프로그램

`설정 > 열기 프로그램`에서 파일이나 폴더를 열 외부 프로그램을 등록할 수 있습니다. 등록한 항목은 선택한 파일/폴더와 확장자 마스크가 맞을 때 메인 메뉴의 `다음으로 열기`와 Windows 컨텍스트 메뉴 상단에 표시됩니다. 내장 뷰어 매핑이 없는 파일은 특정 확장자 마스크와 일치하는 열기 프로그램이 하나뿐이면 더블클릭 시 해당 프로그램으로 열립니다.
...
즐겨찾기 패널 위에 항상 표시되는 컴팩트한 디스크 사용량 도킹 패널입니다.

- 드라이브별 얇은 사용량 바와 남은 용량(GB)을 표시하고, 항목에 마우스를 올리면 총량/사용량/남은 용량 상세 툴팁이 나타납니다.
- 즐겨찾기와의 경계 스플리터로 높이를 조절할 수 있으며, 자동 숨김(고정 핀)이나 숨기기도 가능합니다.
- 패널이 다시 표시될 때 자동으로 새로 고침되며, 컨텍스트 메뉴의 `새로 고침`으로 수동 갱신할 수 있습니다.
- 숨긴 뒤에는 `보기 > 디스크 사용량 미니 패널` 메뉴로 다시 표시합니다.

## 파일 미리보기

- 이미지 파일은 미리보기 영역 크기에 맞춰 표시합니다.
- 일반 파일은 처음 10KB까지 텍스트로 표시하며 바이너리 파일은 안내 문구를 표시합니다.
- 전체 경로, 이름, 형식, MB 단위 크기, 생성·수정일자, 현재 사용자 권한과 파일 특성을 표시합니다.
...
- 즐겨찾기의 `+` 버튼은 파일이 하나 선택되어 있으면 그 파일을, 아니면 현재 활성 폴더를 선택한 그룹에 추가합니다.
- 파일 즐겨찾기는 📄 아이콘으로 표시되며 더블클릭하면 뷰어 탭으로 열립니다. 폴더 전용 메뉴(Explorer에서 열기, 터미널, 새 폴더/새 파일)는 파일 즐겨찾기에서는 숨겨집니다.
- 폴더 즐겨찾기 컨텍스트 메뉴에서 Explorer 열기, PowerShell/CMD 터미널 열기, 새 폴더·새 파일 만들기를 할 수 있습니다.
- 그룹과 즐겨찾기 항목은 컨텍스트 메뉴에서 이름 변경, 순서 이동 및 삭제할 수 있습니다.
- 즐겨찾기 항목은 컨텍스트 메뉴 또는 드래그 앤 드롭으로 다른 그룹에 이동할 수 있습니다.
- 즐겨찾기 패널에서 `Ctrl+C`는 선택한 항목의 경로를 복사합니다.
- 기존 즐겨찾기 설정은 최초 실행 시 `기본` 그룹으로 자동 이관됩니다.
- 문서 탭 끝의 `＋ 새 패널` 탭, 메인 메뉴의 `폴더 패널 추가...` 또는 `Ctrl+T`로 폴더 패널을 추가할 수 있습니다.
...
- `보기 > 기본 도킹 배치로 초기화`로 저장된 배치를 버리고 기본 배치로 되돌릴 수 있습니다.

### 문서 탭 컨텍스트 메뉴

문서 탭(폴더 패널, 뷰어, 콘솔, 디스크 사용량)을 우클릭하면 다음 항목이 나타납니다.

- `닫기`, `다른 탭 닫기`, `왼쪽 탭 닫기`, `오른쪽 탭 닫기`
- `패널 잠금` (아래 참고)
- `탐색기로 열기`: 폴더 패널은 현재 폴더를, 뷰어 탭은 열린 파일을 탐색기에서 선택한 상태로 엽니다. 경로가 없는 탭에는 표시되지 않습니다.

### 패널 잠금

문서 탭을 우클릭한 뒤 `패널 잠금`을 선택하면 그 패널의 닫기를 비활성화할 수 있습니다.

- 잠긴 패널은 탭의 닫기(X) 버튼이 사라지고 컨텍스트 메뉴의 `닫기`가 비활성화되며, 탭 제목 앞에 `🔒` 표시가 붙습니다.
- `다른 탭 닫기`, `왼쪽 탭 닫기`, `오른쪽 탭 닫기`도 잠긴 패널은 건너뜁니다. 콘솔 패널은 패널 안의 닫기 버튼으로도 닫히지 않습니다.
- 잠금 상태는 `%LOCALAPPDATA%\Folderss\panel-locks.xml`에 저장되어 프로그램을 다시 실행해도 유지됩니다.
- 추가한 폴더 패널, Markdown·Monaco·Text 뷰어 탭, 콘솔, 디스크 사용량처럼 닫을 수 있는 모든 문서 탭이 대상입니다. 왼쪽·오른쪽 기본 폴더 탭과 `＋ 새 패널` 탭은 원래 닫을 수 없으므로 메뉴에 나타나지 않습니다.
- 폴더 패널의 잠금은 패널 단위로 기억되므로 패널에서 다른 폴더로 이동해도 유지되고, 뷰어 탭의 잠금은 파일 경로 기준이라 같은 파일을 다시 열면 잠긴 상태로 열립니다.
- `보기 > 기본 도킹 배치로 초기화`를 실행하면 사라진 패널의 잠금 기록도 함께 정리됩니다.

## 테마

메인 메뉴의 `테마` 또는 `설정 > 테마`에서 다음 테마를 전환할 수 있습니다. 마지막 선택은 사용자별로 저장됩니다.
...
| 단축키 | 동작 |
|---|---|
| `F2` | 이름 변경 (즐겨찾기 패널에 포커스가 있으면 즐겨찾기 이름 변경) |
| `F5` | 새로 고침 |
| `F6` | 반대편 패널로 이동 |
| `Delete` | 휴지통으로 이동 |
| `Shift+Delete` | 영구 삭제 |
| `Ctrl+C` | 선택 항목 클립보드 복사 (즐겨찾기 패널에서는 경로 복사) |
| `Ctrl+X` | 선택 항목 클립보드 잘라내기 |
| `Ctrl+V` | 클립보드에서 붙여넣기 (경로 입력란에서는 경로 텍스트 붙여넣기) |
| `Ctrl+N` | 새 파일 |
| `Ctrl+Shift+N` | 새 폴더 |
...
| `Ctrl+T` | 새 폴더 패널 추가 |
| `F11` | 현재 폴더 패널 최대화/복원 |
| `Ctrl+F` | 파일 검색 창 열기 (Markdown·Monaco 뷰어 안에서는 문서 내 검색) |
...
| 위치 | 키 | 동작 |
|---|---|---|
| 파일 목록 | `Enter` | 선택 항목 열기 |
| 파일 목록 | `Backspace` | 상위 폴더로 이동 |
| 파일검색 필터 | `↓` | 파일 목록으로 포커스 이동 |
| 검색 창 | `Enter` / `Esc` | 검색 실행 / 창 닫기 |
| 콘솔 | `Ctrl+C` | 선택 텍스트 복사 또는 인터럽트 |

전체 매핑과 처리 위치는 `docs/keyboard-shortcuts.md`를 참고하세요.
...
SDK 스타일 프로젝트이므로 `dotnet build`를 기본 빌드 명령으로 사용합니다. VS2022의 `MSBuild.exe`를 직접 실행하면 `Microsoft.NET.Sdk` 리졸버 오류가 날 수 있습니다.
...
검색 로직(`SearchService`)은 순수 `System.IO` 코드라 `net8.0` xUnit 프로젝트로 분리해 단독 실행합니다. 앱 솔루션에는 포함되지 않습니다.
...
| 문서 | 내용 |
|---|---|
| `CLAUDE.md` | 개발 가이드, 기능별 수정 체크리스트 |
| `docs/architecture.md` | 파일 구조, 서비스 역할, 확장 포인트 |
| `docs/PROJECT.md` | 개발 아이템 파일 형식과 상태 전환 규칙 |
