...
- 필터 바의 `ignore` 토글로 `.gitignore`/`.folderssignore` 규칙에 직접 해당하는 항목 숨기기 (git 저장소 안에서는 `.git`도 숨김, 상태바에 숨긴 개수, 툴팁에 적용 중인 규칙 파일)
- 심볼릭 링크·junction은 🔗 아이콘과 "링크 대상" 툴팁으로 표시되고 메타정보에도 대상 경로가 나옴
- 메인 메뉴 `반대편 패널에 링크 만들기`: 선택 항목을 가리키는 심볼릭 링크를 반대편 패널 폴더에 만들고, 권한이 없으면 폴더는 junction으로 만듦
...
- 파일 검색 (내용 검색/파일명 검색 대상 선택, 대/소문자 구분, 정규식, 하위 폴더 탐색 옵션)
  ...
  - `패널에 필터 적용`으로 활성 폴더 패널에 검색 결과 파일만 보이게 함. 결과가 있는 하위 폴더로 내려갈 수 있고, 패널 배너의 `해제` 또는 검색 루트 밖으로 이동하면 풀림
...
- Markdown·Monaco 뷰어에서 편집 후 저장하지 않으면 탭 제목 끝에 ` *`가 붙습니다. 그 상태로 탭을 닫으면 변경을 버릴지 묻고, 앱을 종료할 때도 미저장 문서 수를 알려 확인합니다. 저장은 뷰어 안에서 `Ctrl+S`입니다.
...
- 전체 경로, 이름, 형식, MB 단위 크기, 생성·수정일자, 현재 사용자 권한과 파일 특성을 표시합니다. 심볼릭 링크·junction이면 링크 대상 경로도 표시합니다.
