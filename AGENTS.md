...
- `docs/PROJECT.md` — 로컬 아이템 관리 규칙
- `docs/items/` — 현재 개발 아이템과 상태의 단일 기준

### 작업 후 반드시 수행
1. 빌드 성공 확인 (`Exit: 0`)
2. 아래 체크리스트 해당 항목 통과
3. `docs/items/<항목>.md` 본문과 상태를 정확히 반영
4. README.md 관련 섹션 업데이트 (기능 추가/변경 시)

### 작업 상태 및 기록 원칙

- `docs/items/`의 항목별 Markdown 파일을 아이템 상태와 내용의 단일 기준(source of truth)으로 사용한다.
...
- 요구사항, 원인 분석 또는 설계, 구현 내용, 변경 파일, 검증, 변경 이력은 해당 `docs/items/<항목>.md` 본문에 직접 기록한다.
- 상태 전환 시 해당 항목 파일의 `상태` 값만 변경한다.
- 사용자의 추가 수정 요청이 있으면 같은 항목 파일에 요구사항, 구현 및 검증 이력을 보완한다.
- 사용자가 "완료", "확인", "문제없음" 등으로 최종 결과를 승인한 경우에만 항목 상태를 `Done`으로 변경한다.
- 작업 상태, 사용자 확인 여부와 상태 전환일은 `docs/items/<항목>.md`에서만 관리한다.
- 요구사항, 원인 분석, 구현 내용, 변경 파일, 검증 방법과 결과는 항목 파일 본문에서 관리한다.
...
- 항목 파일의 `상태`는 `Todo`, `In Progress`, `Ready for Verification`, `Done` 중 하나만 사용한다.
...
├── PROJECT.md        — 로컬 아이템 운용 규칙
├── items/            — 현재 개발 아이템 상태와 내용을 관리하는 로컬 항목 저장소
...
1. **요청 접수 / 미착수**
   - `docs/items/<항목>.md` 파일을 생성하고 `상태: Todo`로 지정한다.
   - 항목 파일 본문에 상세 문서 필수 항목을 직접 작성한다.

2. **작업 착수 / 개발 및 자체 검증**
   - 항목 파일의 `상태`를 `In Progress`로 변경한다.
   - 항목 파일 본문의 구현 내용과 변경 이력을 갱신한다.

3. **개발 및 자체 검증 완료**
   - 빌드·테스트가 끝나면 항목 파일의 `상태`를 `Ready for Verification`으로 변경한다.
   - 코드 작성과 빌드 성공만으로 DONE 처리하지 않는다.

4. **사용자 최종 확인**
   - 사용자가 결과를 명시적으로 승인한 경우에만 항목 파일의 `상태`를 `Done`으로 변경한다.
...
6. **작업 중 놓친 연관 파일이 있었다면** → `AGENTS.md` 체크리스트 해당 항목에 추가
