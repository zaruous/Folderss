...
- 차단 지점
  ...
  - `MainWindow.Rename_Click` — 잠금 경로의 **폴더** 이름 변경 차단 (파일 이름 변경은 폴더 구조를 바꾸지 않으므로 허용)
...
- [ ] 고정 상태에서 새 폴더/새 파일/폴더 이름 변경/삭제 시 안내 후 차단
- [ ] 고정 상태에서 파일을 선택하고 F2(이름 변경)를 누르면 차단 없이 이름 변경 다이얼로그 표시
...
- 2026-07-02: 버그 수정 — 고정 상태에서 F2로 **파일** 이름 변경까지 차단되던 문제 수정. 파일 이름 변경은 폴더 구조를 바꾸지 않으므로 `Rename_Click`의 잠금 검사를 폴더(`IsDirectory`)에만 적용 (삭제·이동·생성 차단은 기존 그대로 유지)
